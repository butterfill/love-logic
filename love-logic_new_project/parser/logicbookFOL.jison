/*  Defines a JISON lexer and parser for the first-order language
    in Bergmann et al’s The Logic Book (2014).
    
    Differences from Teller:
      - no hats (`a&`)
      - this parser allows predicates like `F2x` (the name can match [A-Z][0-9]*)
      
    Differences from forallx:
      - this parser requires quantifier brackets like  `(all x) Fx`
      - square brackets cannot be used in place of corner brackets
